...
app.layout = html.Div(style=body_style, children=[
    ...
    html.Main([
        ...
        html.P("Across the different industries, we see highly digitalized companies being layoff 'leaders'. We can also see an increasing trend by looking at the visualization below of the total amount of layoffs over time. We can't say for sure whether this trend is due to AI since the super chatbot hype is fairly recent but we should be able to speculate. It all started to become more serious when GPT-4 was announced and released in March of 2023."),
        html.P("The biggest layoff in general was from Google in January 2023 which is visible in the visualization below. In an open letter published by Google CEO Sundar Pichai, the narrative followed a similar trajectory to that of other companies that have downsized in recent months, noting that the company had 'hired for a different economic reality' than what it's up against today. This can make one think about what big megacorp CEOs mean by that. Have they decided to trust emerging AI technologies to automate a big part of their workforce?"),
        dcc.Graph(id='weekly-layoffs-plot'),  # Add a width here # Add a width here
        # Add a hidden interval component
        html.P("The plots show a dangerously high amount of job layoffs in recent months(!). In 'How Artificial Intelligence Could Kill Capitalism', Bernard Marr talks about how the benefits of AI could be accruing to the owners of the technology, leading to a potential rise in economic inequality. He notes that humans will find themselves in a situation where they have to compete for whatever paid jobs are still available to them in a robot-dominated workforce. If we strive to evolve towards a 'Star-Trek-like' post-work utopia, we need a new modern economic system. The transition period towards such a world is at risk of being disastrous economically."),
        html.P("Furthermore, Marr argues that if we can produce goods at a far cheaper cost the owner is still selling his goods to the highest bidder. In a post-work world, it is hard to grasp the concept of a 'highest bidder' due to the new economic situation."),
